...
resource managedGrafanaPrometheusReader 'Microsoft.Authorization/roleAssignments@2022-04-01' = {
  scope: azureMonitorWorkspace
  name: guid(azureMonitorWorkspace.id, managedGrafana.id, 'Monitoring Data Reader')
  properties: {
...
resource deploymentPrincipalGrafanaAdmin 'Microsoft.Authorization/roleAssignments@2022-04-01' = {
  scope: managedGrafana
  name: guid(managedGrafana.id, principalId, 'Grafana Admin')
  properties: {
...
output azureMonitorWorkspaceLocation string = azureMonitorWorkspace.location
