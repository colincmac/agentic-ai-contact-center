...
public static class DemoRegistration
{
    public static BankingDemoOptions AddBankingDemo(this WebApplicationBuilder builder)
    {
        ...
        builder.Services.AddOptions<BankingDemoOptions>().Bind(configuration).ValidateOnStart();
        ...
        if (validation.Failed) { throw new OptionsValidationException("", typeof(BankingDemoOptions), validation.Failures); }
        ...
        builder.Services.AddAzureSpeech(speech =>
        {
            ...
            speech.OutputFormat = Microsoft.CognitiveServices.Speech.SpeechSynthesisOutputFormat.Raw24Khz16BitMonoPcm;
        });
        builder.Services.RemoveAll<ISpeechSynthesizer>();
        builder.Services.AddScoped<ISpeechSynthesizer, BankingPromptSynthesizer>();
        builder.Services.AddSingleton<IRealtimeClient>(sp =>
            new AzureVoiceLiveClient(options.VoiceLiveEndpoint!, credential, options.VoiceLiveModel)
                .AsBuilder().UseFunctionInvocation(sp.GetRequiredService<ILoggerFactory>(), pipeline =>
                {
        ...
        builder.Services.AddKeyedSingleton<RealtimeAIAgent>("bank-demo", (sp, _) =>
            new RealtimeAIAgent(sp.GetRequiredService<IRealtimeClient>(), new RealtimeAgentOptions
            {
                ...
                SessionOptions = new RealtimeSessionOptions
                {
                    ...
                    RawRepresentationFactory = () => new Azure.AI.VoiceLive.VoiceLiveSessionOptions
                    {
                        InputAudioSamplingRate = 24000,
                        TurnDetection = new Azure.AI.VoiceLive.ServerVadTurnDetection { CreateResponse = true, InterruptResponse = true },
                    },
                },
            }, sp.GetRequiredService<ILoggerFactory>()));

        builder.Services.AddSingleton<DemoBankingService>();
        builder.Services.AddSingleton<IDemoBankingService>(sp => sp.GetRequiredService<DemoBankingService>());
        builder.Services.AddSingleton<ICallerDirectory>(sp => sp.GetRequiredService<DemoBankingService>());
